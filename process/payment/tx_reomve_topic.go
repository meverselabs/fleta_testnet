...
func (tx *RemoveTopic) From() common.Address {
	return tx.From_
}

// Validate validates signatures of the transaction
func (tx *RemoveTopic) Validate(p types.Process, loader types.LoaderWrapper, signers []common.PublicHash) error {
	sp := p.(*Payment)
...
func (tx *RemoveTopic) Execute(p types.Process, ctw *types.ContextWrapper, index uint16) error {
	sp := p.(*Payment)

	sp.removeTopic(ctw, tx.Topic)
	return nil
}

// MarshalJSON is a marshaler function
func (tx *RemoveTopic) MarshalJSON() ([]byte, error) {
