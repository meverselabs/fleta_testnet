...
func (tx *CreateOmega) Fee(p types.Process, loader types.LoaderWrapper) *amount.Amount {
	sp := p.(*Formulator)
...
func (tx *CreateOmega) Validate(p types.Process, loader types.LoaderWrapper, signers []common.PublicHash) error {
	...
	for i, From := range tx.SigmaFormulators {
		...
		if sp.IsRewardBaseUpgrade(loader) {
			if frAcc.RewardCount*rewardPolicy.PayRewardEveryBlocks+frAcc.PreHeight < policy.OmegaRequiredSigmaBlocks {
